...
import { api, MechanicType as APIMechanicType } from '@/services/api';
import { useSystemConfig } from '@nexone/ui';

export default function MechanicTypePage() {
    const [data, setData] = useState<APIMechanicType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = useState<APIMechanicType | null>(null);
    const [formData, setFormData] = useState<Partial<APIMechanicType>>({ name: '', description: '', status: 'active' });
    const [formErrors, setFormErrors] = useState<{ [key: string]: string }>({});

    const loadData = () => {
        api.getMechanicTypes().then(res => setData((res || []) as unknown as APIMechanicType[])).catch(() => setData([]));
    };
    useEffect(() => { loadData(); }, []);
    ...
    const handleEdit = (item: APIMechanicType) => {
    ...
    const handleView = (item: APIMechanicType) => {
        setFormData({ name: item.name, description: item.description, status: item.status });
        setSelectedItem(item);
        setFormErrors({});
        setModalMode('view');
    ...
    const handleDelete = (item: APIMechanicType) => {
        setSelectedItem(item);
        setModalMode('delete');
        setIsModalOpen(true);
    };
    ...
    const saveForm = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await api.createMechanicType({ name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            } else if (modalMode === 'edit' && selectedItem) {
                await api.updateMechanicType(selectedItem.id, { name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            }
            loadData();
    ...
    const confirmDelete = async () => {
        try {
            if (selectedItem) { await api.deleteMechanicType(selectedItem.id); loadData(); }
        } catch (e) { console.error(e); }
        setIsModalOpen(false);
    };
    ...
    return (
        <CrudLayout
            ...
            toolbarRight={
                <>
                    <SearchInput value={search} onChange={setSearch} placeholder="ค้นหาประเภทช่างซ่อม..." />
        ...
        >
                                ...
                                <td className="text-center">
                                    <StatusDropdown 
                                        ...
                                        onChange={async (val: any) => { 
                                            ...
                                            try { await api.updateMechanicType(item.id, { ...item, status: val } as any); } catch(err){}
                                        }}
                                        options={[
            ...
            <BaseModal 
                ...
                title={modalMode === 'add' ? 'เพิ่มประเภทช่างซ่อม' : modalMode === 'edit' ? 'แก้ไขประเภทช่างซ่อม' : 'รายละเอียดประเภทช่างซ่อม'}
                footer={
                    modalMode !== 'view' ? (
                        <>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                        <label style={crudStyles.label}>ชื่อประเภทช่างซ่อม <span style={{color: '#ef4444'}}>*</span></label>
                        <input 
                            type="text" 
                            style={{ ...crudStyles.input, borderColor: formErrors.name ? '#ef4444' : 'var(--border-color)' }} 
                            placeholder="ระบุชื่อประเภทช่างซ่อม"
                            value={formData.name || ''}
                            onChange={(e) => {
