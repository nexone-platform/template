...
import { api, VehicleType as APIVehicleType } from '@/services/api';

export default function VehicleTypePage() {
    const [data, setData] = useState<APIVehicleType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = useState<APIVehicleType | null>(null);
    const [formData, setFormData] = useState<Partial<APIVehicleType>>({ name: '', description: '', status: 'active' });
    const [formErrors, setFormErrors] = useState<{ [key: string]: string }>({});

    const loadData = () => {
        api.getVehicleTypes().then(res => setData((res || []) as unknown as APIVehicleType[])).catch(() => setData([]));
    };
    useEffect(() => { loadData(); }, []);
    ...
    const handleEdit = (item: APIVehicleType) => {
    ...
    const handleView = (item: APIVehicleType) => {
        setFormData({ name: item.name, description: item.description, status: item.status });
        setSelectedItem(item);
        setFormErrors({});
        setModalMode('view');
    ...
    const handleDelete = (item: APIVehicleType) => {
        setSelectedItem(item);
        setModalMode('delete');
        setIsModalOpen(true);
    };
    ...
    const saveForm = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await api.createVehicleType({ name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            } else if (modalMode === 'edit' && selectedItem) {
                await api.updateVehicleType(selectedItem.id, { name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            }
            loadData();
    ...
    const confirmDelete = async () => {
        try {
            if (selectedItem) { await api.deleteVehicleType(selectedItem.id); loadData(); }
        } catch (e) { console.error(e); }
        setIsModalOpen(false);
    };
    ...
    return (
        <CrudLayout
            ...
            toolbarRight={
                <>
                    <SearchInput value={search} onChange={setSearch} placeholder="ค้นหาประเภทรถ..." />
        ...
        >
                                ...
                                <td className="text-center">
                                    <StatusDropdown 
                                        ...
                                        onChange={async (val: any) => { 
                                            ...
                                            try { await api.updateVehicleType(item.id, { ...item, status: val } as any); } catch(err){}
                                        }}
                                        options={[
            ...
            <BaseModal 
                ...
                title={modalMode === 'add' ? 'เพิ่มประเภทรถ' : modalMode === 'edit' ? 'แก้ไขประเภทรถ' : 'รายละเอียดประเภทรถ'}
                footer={
                    modalMode !== 'view' ? (
                        <>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                        <label style={crudStyles.label}>ชื่อประเภทรถ <span style={{color: '#ef4444'}}>*</span></label>
                        <input 
                            type="text" 
                            style={{ ...crudStyles.input, borderColor: formErrors.name ? '#ef4444' : 'var(--border-color)' }} 
                            placeholder="ระบุชื่อประเภทรถ"
                            value={formData.name || ''}
                            onChange={(e) => {
