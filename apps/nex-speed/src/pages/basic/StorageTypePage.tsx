...
import { api, StorageType as APIStorageType } from '@/services/api';
import { useSystemConfig } from '@nexone/ui';

export default function StorageTypePage() {
    const [data, setData] = useState<APIStorageType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = useState<APIStorageType | null>(null);
    const [formData, setFormData] = useState<Partial<APIStorageType>>({ name: '', description: '', status: 'active' });
    const [formErrors, setFormErrors] = useState<{ [key: string]: string }>({});

    const loadData = () => {
        api.getStorageTypes().then(res => setData((res || []) as unknown as APIStorageType[])).catch(() => setData([]));
    };
    useEffect(() => { loadData(); }, []);
    ...
    const handleEdit = (item: APIStorageType) => {
    ...
    const handleView = (item: APIStorageType) => {
        setFormData({ name: item.name, description: item.description, status: item.status });
        setSelectedItem(item);
        setFormErrors({});
        setModalMode('view');
    ...
    const handleDelete = (item: APIStorageType) => {
        setSelectedItem(item);
        setModalMode('delete');
        setIsModalOpen(true);
    };
    ...
    const saveForm = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await api.createStorageType({ name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            } else if (modalMode === 'edit' && selectedItem) {
                await api.updateStorageType(selectedItem.id, { name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            }
            loadData();
    ...
    const confirmDelete = async () => {
        try {
            if (selectedItem) { await api.deleteStorageType(selectedItem.id); loadData(); }
        } catch (e) { console.error(e); }
        setIsModalOpen(false);
    };
    ...
    return (
        <CrudLayout
            ...
            toolbarRight={
                <>
                    <SearchInput value={search} onChange={setSearch} placeholder="ค้นหาประเภทคลัง/สถานที่..." />
        ...
        >
                                ...
                                <td className="text-center">
                                    <StatusDropdown 
                                        ...
                                        onChange={async (val: any) => { 
                                            ...
                                            try { await api.updateStorageType(item.id, { ...item, status: val } as any); } catch(err){}
                                        }}
                                        options={[
            ...
            <BaseModal 
                ...
                title={modalMode === 'add' ? 'เพิ่มประเภทคลัง/สถานที่' : modalMode === 'edit' ? 'แก้ไขประเภทคลัง/สถานที่' : 'รายละเอียดประเภทคลัง/สถานที่'}
                footer={
                    modalMode !== 'view' ? (
                        <>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                        <label style={crudStyles.label}>ชื่อประเภทคลัง/สถานที่ <span style={{color: '#ef4444'}}>*</span></label>
                        <input 
                            type="text" 
                            style={{ ...crudStyles.input, borderColor: formErrors.name ? '#ef4444' : 'var(--border-color)' }} 
                            placeholder="ระบุชื่อประเภทคลัง/สถานที่"
                            value={formData.name || ''}
                            onChange={(e) => {
