...
export default function UnitTypePage() {
    ...
    const [selectedItem, setSelectedItem] = useState<APIUnitType | null>(null);
    const [formData, setFormData] = useState<Partial<APIUnitType>>({ name: '', description: '', status: 'active' });
    const [formErrors, setFormErrors] = useState<{ [key: string]: string }>({});

    const loadData = () => {
        api.getUnitTypes().then(res => setData((res || []) as unknown as APIUnitType[])).catch(() => setData([]));
    };
    useEffect(() => { loadData(); }, []);
    ...
    const handleEdit = (item: APIUnitType) => {
    ...
    const handleView = (item: APIUnitType) => {
        setFormData({ name: item.name, description: item.description, status: item.status });
        setSelectedItem(item);
        setFormErrors({});
        setModalMode('view');
    ...
    const handleDelete = (item: APIUnitType) => {
        setSelectedItem(item);
        setModalMode('delete');
        setIsModalOpen(true);
    };
    ...
    const saveForm = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await api.createUnitType({ name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            } else if (modalMode === 'edit' && selectedItem) {
                await api.updateUnitType(selectedItem.id, { name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            }
            loadData();
    ...
    const confirmDelete = async () => {
        try {
            if (selectedItem) { await api.deleteUnitType(selectedItem.id); loadData(); }
        } catch (e) { console.error(e); }
        setIsModalOpen(false);
    };
                                ...
                                <td className="text-center">
                                    <StatusDropdown 
                                        ...
                                        onChange={async (val: any) => { 
                                            ...
                                            try { await api.updateUnitType(item.id, { ...item, status: val } as any); } catch(err){}
                                        }}
                                        options={[
