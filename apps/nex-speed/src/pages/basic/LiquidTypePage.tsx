...
import { api, LiquidType as APILiquidType } from '@/services/api';
import { useSystemConfig } from '@nexone/ui';

export default function LiquidTypePage() {
    const [data, setData] = useState<APILiquidType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = useState<APILiquidType | null>(null);
    const [formData, setFormData] = useState<Partial<APILiquidType>>({ name: '', description: '', status: 'active' });
    const [formErrors, setFormErrors] = useState<{ [key: string]: string }>({});

    const loadData = () => {
        api.getLiquidTypes().then(res => setData((res || []) as unknown as APILiquidType[])).catch(() => setData([]));
    };
    useEffect(() => { loadData(); }, []);
    ...
    const handleEdit = (item: APILiquidType) => {
    ...
    const handleView = (item: APILiquidType) => {
        setFormData({ name: item.name, description: item.description, status: item.status });
        setSelectedItem(item);
        setFormErrors({});
        setModalMode('view');
    ...
    const handleDelete = (item: APILiquidType) => {
        setSelectedItem(item);
        setModalMode('delete');
        setIsModalOpen(true);
    };
    ...
    const saveForm = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await api.createLiquidType({ name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            } else if (modalMode === 'edit' && selectedItem) {
                await api.updateLiquidType(selectedItem.id, { name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            }
            loadData();
    ...
    const confirmDelete = async () => {
        try {
            if (selectedItem) { await api.deleteLiquidType(selectedItem.id); loadData(); }
        } catch (e) { console.error(e); }
        setIsModalOpen(false);
    };
    ...
    return (
        <CrudLayout
            ...
            toolbarRight={
                <>
                    <SearchInput value={search} onChange={setSearch} placeholder="ค้นหาประเภทของเหลว..." />
        ...
        >
                                ...
                                <td className="text-center">
                                    <StatusDropdown 
                                        ...
                                        onChange={async (val: any) => { 
                                            ...
                                            try { await api.updateLiquidType(item.id, { ...item, status: val } as any); } catch(err){}
                                        }}
                                        options={[
            ...
            <BaseModal 
                ...
                title={modalMode === 'add' ? 'เพิ่มประเภทของเหลว' : modalMode === 'edit' ? 'แก้ไขประเภทของเหลว' : 'รายละเอียดประเภทของเหลว'}
                footer={
                    modalMode !== 'view' ? (
                        <>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                        <label style={crudStyles.label}>ชื่อประเภทของเหลว <span style={{color: '#ef4444'}}>*</span></label>
                        <input 
                            type="text" 
                            style={{ ...crudStyles.input, borderColor: formErrors.name ? '#ef4444' : 'var(--border-color)' }} 
                            placeholder="ระบุชื่อประเภทของเหลว"
                            value={formData.name || ''}
                            onChange={(e) => {
