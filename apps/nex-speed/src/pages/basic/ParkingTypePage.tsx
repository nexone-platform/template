...
import { api, ParkingType as APIParkingType } from '@/services/api';

export default function ParkingTypePage() {
    const [data, setData] = useState<APIParkingType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = useState<APIParkingType | null>(null);
    const [formData, setFormData] = useState<Partial<APIParkingType>>({ name: '', description: '', status: 'active' });
    const [formErrors, setFormErrors] = useState<{ [key: string]: string }>({});

    const loadData = () => {
        api.getParkingTypes().then(res => setData((res || []) as unknown as APIParkingType[])).catch(() => setData([]));
    };
    useEffect(() => { loadData(); }, []);
    ...
    const handleEdit = (item: APIParkingType) => {
    ...
    const handleView = (item: APIParkingType) => {
        setFormData({ name: item.name, description: item.description, status: item.status });
        setSelectedItem(item);
        setFormErrors({});
        setModalMode('view');
    ...
    const handleDelete = (item: APIParkingType) => {
        setSelectedItem(item);
        setModalMode('delete');
        setIsModalOpen(true);
    };
    ...
    const saveForm = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await api.createParkingType({ name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            } else if (modalMode === 'edit' && selectedItem) {
                await api.updateParkingType(selectedItem.id, { name: formData.name!, description: formData.description || '', status: formData.status || 'active' });
            }
            loadData();
    ...
    const confirmDelete = async () => {
        try {
            if (selectedItem) { await api.deleteParkingType(selectedItem.id); loadData(); }
        } catch (e) { console.error(e); }
        setIsModalOpen(false);
    };
    ...
    return (
        <CrudLayout
            ...
            toolbarRight={
                <>
                    <SearchInput value={search} onChange={setSearch} placeholder="ค้นหาประเภทลานจอดรถ..." />
                    <button onClick={handleAdd} className="btn btn-primary btn-sm" style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--accent-blue)', color: 'white', border: 'none', borderRadius: '8px', padding: '8px 16px', fontWeight: 500, cursor: 'pointer' }}><Plus size={16} /> <span>เพิ่มประเภทลานจอดรถ</span></button>
                </>
            }
        >
                                ...
                                <td className="text-center">
                                    <StatusDropdown 
                                        ...
                                        onChange={async (val: any) => { 
                                            ...
                                            try { await api.updateParkingType(item.id, { ...item, status: val } as any); } catch(err){}
                                        }}
                                        options={[
            ...
            <BaseModal 
                ...
                title={modalMode === 'add' ? 'เพิ่มประเภทลานจอดรถ' : modalMode === 'edit' ? 'แก้ไขประเภทลานจอดรถ' : 'รายละเอียดประเภทลานจอดรถ'}
                footer={
                    modalMode !== 'view' ? (
                        <>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                        <label style={crudStyles.label}>ชื่อประเภทลานจอดรถ <span style={{color: '#ef4444'}}>*</span></label>
                        <input 
                            type="text" 
                            style={{ ...crudStyles.input, borderColor: formErrors.name ? '#ef4444' : 'var(--border-color)' }} 
                            placeholder="ระบุชื่อประเภทลานจอดรถ"
                            value={formData.name || ''}
                            onChange={(e) => {
