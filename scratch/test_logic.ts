...
const handleToggleUse = (menuId: string, currentTree: MenuNode[]): MenuNode[] => {
  ...
  const mType = target.menuType || (target as any).menu_type;
    ...
    const findAndAddSiblings = (nodes: MenuNode[]) => {
      ...
      if (idx !== -1) {
        for (let i = idx + 1; i < nodes.length; i++) {
          const sibType = nodes[i].menuType || (nodes[i] as any).menu_type;
